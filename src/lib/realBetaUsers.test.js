/**
 * Test BudE Matching Algorithm with Real Beta Users
 * 27 users from actual beta testing
 */

import { calculateCompatibility, findMatches } from './matchingAlgorithm.js';

// Parse real beta user data (cleaned and structured)
const betaUsers = [
  {
    id: 'user_014',
    firstName: 'Jeff',
...
console.log('========================================');
console.log('REAL BETA USERS - MATCHING ANALYSIS');
console.log(`Total Users: ${betaUsers.length}`);
console.log('========================================\n');

// Generate matching matrix for all users
const matchingResults = [];

betaUsers.forEach((user, index) => {
  const otherUsers = betaUsers.filter((_, i) => i !== index);
  const matches = findMatches(user, otherUsers, 10);
  ...
  matchingResults.push({
    ...
    topMatches: matches.slice(0, 5).map(m => ({
      ...
      score: m.score,
      topReason: m.matches.organizations.details[0] ||
                m.matches.networkingGoals.details[0] ||
                m.matches.professionalInterests.details[0] ||
                'Various shared interests'
    }))
  });
});

// Sort by most matches to least
matchingResults.sort((a, b) => b.totalMatches - a.totalMatches);

console.log('TOP NETWORKERS (Most potential matches above 70%):\n');
matchingResults.slice(0, 10).forEach((result, i) => {
  console.log(`${i + 1}. ${result.user} - ${result.totalMatches} matches`);
  if (result.topMatches.length > 0) {
    result.topMatches.forEach((match, j) => {
      console.log(`   ${j + 1}) ${match.name} (${match.score}%) - ${match.topReason}`);
    });
  } else {
...
console.log('\n========================================');
console.log('USERS WITH NO MATCHES (Need profile improvement):');
console.log('========================================\n');

const noMatches = matchingResults.filter(r => r.totalMatches === 0);
...
const totalMatches = matchingResults.reduce((sum, r) => sum + r.totalMatches, 0);
const avgMatches = (totalMatches / betaUsers.length).toFixed(1);
const usersWithMatches = matchingResults.filter(r => r.totalMatches > 0).length;
const matchRate = ((usersWithMatches / betaUsers.length) * 100).toFixed(1);

console.log(`Average matches per user: ${avgMatches}`);
console.log(`Users with at least 1 match: ${usersWithMatches}/${betaUsers.length} (${matchRate}%)`);
console.log(`Users with 5+ matches: ${matchingResults.filter(r => r.totalMatches >= 5).length}`);
console.log(`Users with 10+ matches: ${matchingResults.filter(r => r.totalMatches >= 10).length}`);

console.log('\n========================================');
console.log('QUALITY OVER QUANTITY CHECK:');
console.log('========================================\n');

const targetRange = betaUsers.length * 0.05; // 5% of users
const targetRangeMax = betaUsers.length * 0.10; // 10% of users
console.log(`Target: 5-10% of users (${Math.round(targetRange)}-${Math.round(targetRangeMax)} matches per user)`);
console.log(`Actual average: ${avgMatches} matches per user`);
console.log(avgMatches >= targetRange && avgMatches <= targetRangeMax ? '✅ Within target range!' : '⚠️ Outside target range');
