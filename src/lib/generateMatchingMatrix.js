/**
 * Generate full compatibility matrix as CSV
 */

import { calculateCompatibility } from './matchingAlgorithm.js';
import fs from 'fs';

// Beta users data (same as realBetaUsers.test.js)
const betaUsers = [
  {
    id: 'user_014',
    firstName: 'Jeff',
...
const matrix = [];
const headers = ['User', ...betaUsers.map(u => `${u.firstName} ${u.lastName}`)];
matrix.push(headers.join(','));

betaUsers.forEach(user1 => {
  const row = [`${user1.firstName} ${user1.lastName}`];

  betaUsers.forEach(user2 => {
...
const outputPath = 'C:\\Users\\Jeff\\OneDrive\\Desktop\\bude-matching-matrix.csv';
fs.writeFileSync(outputPath, csvContent);

console.log(`✅ Compatibility matrix saved to: ${outputPath}`);
console.log(`\nMatrix size: ${betaUsers.length} x ${betaUsers.length}`);
console.log('You can open this in Excel or Google Sheets to see all compatibility scores!');
