 ...
 * Run this with: node test-chantel-steve-matches.cjs
 */
...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1vcWhnaGJxYXBjcHB6eWRncXl0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE0NDczNzMsImV4cCI6MjA3NzAyMzM3M30.8WjiloL6xF-u_1vydb_7QMyGWc8c0vHgmt5NyQoNZzc';
...
function testUserMatches(testUser, testUserRaw, users) {
  ...
  console.log(`\n📊 Profile Data:`);
  console.log(`  Organizations Current: ${JSON.stringify(testUserRaw.organizations_current) || 'None'}`);
  console.log(`  Organizations Interested: ${JSON.stringify(testUserRaw.organizations_interested) || 'None'}`);
  console.log(`  Networking Goals: ${testUserRaw.networking_goals || 'None'}`);
  console.log(`  Professional Interests: ${JSON.stringify(testUserRaw.professional_interests) || 'None'}`);
  console.log(`  Personal Interests: ${JSON.stringify(testUserRaw.personal_interests) || 'None'}`);
